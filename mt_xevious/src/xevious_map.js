...
export const _MAP={
	_background_speed:0.5,
	_map:new Object(),
	_t:30,
	init(_p){
		_GAME_COMMON._set_ajax({
			url:'./xevious_map.json'
		}).then(function(_d){
			_MAP._map = _d;
			_p.f();
		});
	...
	_set_gamestart(){
		...
		let _om=_this._map[_i];
		//MAP縦
		for(let _j=0;_j<_om._map.length;_j++){
			//MAP横
			let _maps=_om._map[_j];
			for(let _k=0;_k<_maps.length;_k++){
				if(_maps[_k]==='0'){continue;}
				if(_maps[_k]===undefined){continue;}
				let _length = _om._map.length;
				let _d=(-1*_length*_this._t)+(_j*_this._t);
				_XPE._PARTS_ENEMIESMAIN._map_def[_maps[_k]]
					._gamestart(_k*_this._t,_d)
			}//_k
